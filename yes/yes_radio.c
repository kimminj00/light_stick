...
#define test_buf_length 256

static const uint8_t test[test_length] = {0x88,0x41,0x01,0x24,0x75,0xFF,0xFF,0x00,0x00,0x4d,\
		                                  0x01,0x00,0x01,0x0A,0x00,0x02,0x00,0x00,0x00,0x38,\
										  0x02,0x18,0x02,0x01,0x00,0x00,0x00,0x00,0x00,0x00,\
										  0x00,0x00,0x00,0x41,0x03,0x00,0x00,0x00,0x00,0x01,\
										  0x00,0x00,0x00,0x00,0x00,0x64};
static uint8_t testBuffer[test_buf_length];
static uint16_t length_test=0;

static uint8_t S_F1=0x88;
static uint8_t S_F2=0x41;
static uint8_t S_F3=0;
static uint8_t S_F4=0x24;
static uint8_t S_F5=0x75;
static uint8_t S_F6=0xFF;
static uint8_t S_F7=0xFF;
static uint8_t S_F8=0x00;
static uint8_t S_F9=0x00;
...
void SendMessageIEEE802154(uint16_t dest, uint8_t* data, uint8_t length)
{
	...
	testBuffer[0]=length+10;

	testBuffer[1]=S_F1;
	testBuffer[2]=S_F2;
	testBuffer[3]=++txCount;
	testBuffer[4]=S_F4;
	testBuffer[5]=S_F5;
	testBuffer[6]=S_F6;
	testBuffer[7]=S_F7;
	...
	{
		testBuffer[i]=data[i-10];
		//printf("%d: 0x%2X \r\n",i,data[i]);
	}
	length_test = length+10;
	...
	if (txPacket != NULL)
	{
		...
		{
			//printf("reTxCount802154 : %d \r\n", reTxCount802154);
			//RAIL_SetTimer(railHandle2p4, 2000 * 10, RAIL_TIME_DELAY,&ReTx_TimerExpired);
			//RAIL_SetTimer(railHandle2p4, 10 * 6, RAIL_TIME_DELAY,&ReTx_TimerExpired);

			//RAIL_SetTimer(railHandle2p4, 2000 * 10, RAIL_TIME_DELAY,&ReTx_TimerExpired);
			RAIL_SetTimer(railHandle2p4, 1000 * (gReTxtime802154), RAIL_TIME_DELAY,&ReTx_TimerExpired);
			reTxCount802154++;
		}
...
void ReTx_TimerExpired(RAIL_Handle_t railHandle)
{
	...
	if (re2p4Packet != NULL) {
		...
		if (reTxCount802154 < gReTxCount802154 - 1) {
			//printf("reTxCount802154 : %d \r\n", reTxCount802154);
			RAIL_SetTimer(railHandle2p4, 1000 * (gReTxtime802154), RAIL_TIME_DELAY,
					&ReTx_TimerExpired);
			reTxCount802154++;
		}
	}
...
void Tx2p4GPending(void)
{
	...
	memcpy(tx_packet, &testBuffer[0], test_length);
	...
	if(tx_packet != NULL)
	{
		//if(testBuffer[0] != 0)
		{
			...
			//RAIL_SetTxFifo(railHandle2p4, test,test_length, length_test+1);

			RAIL_WriteTxFifo(railHandle2p4, (uint8_t *)tx_packet, test_length, true);

			//RAIL_WriteTxFifo(railHandle2p4, &testBuffer[0], length_test, true);

			//RAIL_StartTx(railHandle2p4, gChannel_2p4, RAIL_TX_OPTIONS_DEFAULT, NULL);

			//RAIL_StartCcaCsmaTx(railHandle2p4,gChannel_2p4, RAIL_TX_OPTIONS_DEFAULT, &CsmaConfig, NULL);
		...
		//printf("Data check  len = %d, %d\r\n",length_test,test_buf_length);
	...
	if(!queueIsEmpty(&txPacketQueue))
	{
		...
		if (txPacket != NULL)
		{
			...
			uint8_t *payload = get2p4Data(txPacket);
			// for (uint16_t i = 0; i < (txPacket->len - sizeof(MAC_SADDR_HEADER_t)); i++) {
...
void SendMessageIEEE802154(uint16_t dest, uint8_t* data, uint8_t length) {
	...
	header->frameCtrl = FRAME_CONTROL;
		header->dstPanId = PAN_ID;
		header->srcAddr = gNodeID;
		header->srcAddr = gNodeId;
...
void initRadio802154(void)
{
  ...
  // Initialize the PA now that the HFXO is up and the timing is correct
   ...
   // RAIL_EVENT_RX_PACKET_RECEIVED | | RAIL_EVENT_RX_ACK_TIMEOUT
...
//			printPacket("rxPacket", rxPacketData->dataPtr, rxPacketData->dataLength, rxPacketData);
			...
			if (header->seqNum == rxSquenceNum) {		// �ߺ� �޽��� �� ���
				memoryFree(rxPacketHandle);
